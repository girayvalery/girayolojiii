...
function CommentNode({
  ...
  return (
    <div
      className={`${indent > 0 ? "ml-4 pl-3" : ""}`}
      style={{ borderLeft: indent > 0 ? "2px solid #33333355" : "none" }}
    >
      <div className="py-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          <Link
            href={`/profile/${comment.author.id}`}
            className="text-xs font-semibold hover:text-green-500"
        ...
        {!collapsed && (
          <>
            <p
              className="text-sm leading-relaxed mb-2 pl-9"
            ...
            <div className="flex items-center gap-3 pl-9">
              ...
              <button
                onClick={startReply}
                className="text-xs font-medium hover:text-green-500"
                style={{ color: "var(--text-muted)" }}
              >
                💬 Yanıtla
              </button>
            ...
            {replying && (
              <div className="mt-3 pl-9">
                <textarea
                  ...
                  className="auth-input resize-none mb-2 text-sm"
                  autoFocus
                />
  ...
  async function load() {
    try {
      ...
      function attach(node: Comment): Comment {
        return {
          ...node,
          replies: flat.filter((c) => c.parentId === node.id).map(attach),
        };
      }

...
export default function AdminClient() {
    ...
    try {
      const [subs, posts, usersList, vids, fb] = await Promise.all([
        ...
        fetch("/api/db/users", { cache: "no-store" })
          .then((r) => r.json())
          .catch(() => []),
        fetch("/api/db/videos", { cache: "no-store" })
          .then((r) => r.json())
          .catch(() => []),
  ...
  async function deletePost(post: any) {
    ...
    setPublished((p) =>
      p.filter((x) => (x._id || x.id) !== (post._id || post.id)),
    );
  }

  async function changeRole(
    userId: string,
    newRole: "ADMIN" | "YAZAR" | "UYE",
  ) {
    ...
    setUsers((p) =>
      p.map((u) => (u.id === userId ? { ...u, role: newRole } : u)),
    );
  }
    ...
    <div className="min-h-screen" style={{ background: "var(--bg)" }}>
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex gap-2 mb-6 flex-wrap overflow-x-auto pb-2">
          {[
            { key: "overview" as const, icon: "📊", label: "Genel" },
            {
              ...
              icon: "⏳",
              label: `Bekleyen (${pendingList.length})`,
            },
            {
              key: "published" as const,
              icon: "✅",
              label: `Yazılar (${published.length})`,
            },
            {
              key: "users" as const,
              icon: "👥",
              label: `Üyeler (${users.length})`,
            },
            {
              key: "videos" as const,
              icon: "🎬",
              label: `Videolar (${videos.length})`,
            },
            {
              key: "reels" as const,
              icon: "⚡",
            ...
            },
            {
              ...
              icon: "📸",
              label: `Hikayeler (${stories.length})`,
            },
            {
              key: "feedback" as const,
              icon: "💌",
              label: `Geri Bildirim (${feedback.length})`,
            },
          ...
          <>
            ...
            <div className="space-y-3">
              {filteredUsers.map((u) => (
                <div
                  key={u.id}
                  className="flex items-center gap-4 rounded-xl p-4 flex-wrap"
                  style={{
                    background: "var(--bg-card)",
                    border: "1px solid var(--border)",
                  }}
            ...
            {videos.map((v: any) => (
              ...
              >
                {v.youtubeId ? (
                  ...
                  <div
                    className={`relative h-32 flex items-center justify-center bg-gradient-to-br ${v.bgGradient || "from-gray-800 to-gray-900"}`}
                  >
                ...
                <div className="p-4">
                  ...
                  <p
                    ...
                    style={{ color: "var(--text-muted)" }}
                  >
                    {(v.views || 0).toLocaleString()} görüntülenme
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {reels.length === 0 ? (
              ...
              reels.map((r) => (
                ...
                >
                  <div
                    className={`relative h-40 flex items-center justify-center bg-gradient-to-b ${r.bgGradient}`}
                  >
        ...
        {!loading && tab === "settings" && (
          <div
            className="rounded-2xl p-6 space-y-4 max-w-2xl"
            style={{
              background: "var(--bg-card)",
              border: "1px solid var(--border)",
            }}
          ...
          >
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                style={{ background: "#1f1f1f", color: "#bbb" }}
              >
                İptal
              </button>
            </div>

...
async function findPost(db: any, id: string) {
  try {
    const byObjectId = await db.collection('posts').findOne({ _id: new ObjectId(id) })
...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const allowed = ['title', 'content', 'excerpt', 'category', 'tags', 'coverEmoji', 'coverImage', 'youtubeId']

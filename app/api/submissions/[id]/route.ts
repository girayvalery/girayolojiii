...
export async function PATCH(
  ...
  try {
    ...
    if (action === "approve") {
      ...
      const dbUser = sub.userId
        ? await db.collection("users").findOne({ id: sub.userId })
        : null;
      ...
      if (type === "video") {
        ...
        await db.collection("videos").insertOne(vid);
        try {
          await db
            .collection("submissions")
            .deleteOne({ _id: new ObjectId(params.id) });
        ...
        return NextResponse.json({ message: "Video onaylandı", video: vid });
      }

...
export default function ProfilePage({ params }: { params: { id: string } }) {
  ...
  const load = useCallback(async () => {
    try {
      const usersRes = await fetch('/api/db/users', { cache: 'no-store' }).then(r => r.json())
      const u = Array.isArray(usersRes) ? usersRes.find((x: any) => x.id === params.id) : null
      ...
      const postsRes = await fetch('/api/db/posts', { cache: 'no-store' }).then(r => r.json())
      const myPosts = Array.isArray(postsRes) ? postsRes.filter((p: any) => p.author?.id === params.id) : []
      setPosts(myPosts)

      const ach = await fetch(`/api/achievements?userId=${params.id}`, { cache: 'no-store' }).then(r => r.json())
  ...
  if (!user) return (
    <div className="max-w-screen-xl mx-auto px-4 py-20 text-center">
      ...
      <h1 className="text-2xl font-semibold mb-2" style={{ color: 'var(--text)' }}>Kullanıcı bulunamadı</h1>
      <Link href="/" className="inline-block mt-6 px-5 py-2 rounded-full text-sm font-semibold text-white" style={{ background: '#1D9E75' }}>Ana Sayfa</Link>
    </div>
  )
  ...
  async function saveAvatarConfig(cfg: AvatarConfig) {
    try {
      ...
      if (res.ok) {
        // Session'ı NextAuth update() ile yenile - JWT 'update' trigger DB'den çekecek
        try { await update({ trigger: 'update' }) } catch {}
        show('success', '🎨 Karakter kaydedildi!')
        // Hard reload - yeni JWT için
  ...
  async function savePhotoUrl(url: string) {
    try {
      ...
      if (res.ok) {
        try { await update({ trigger: 'update' }) } catch {}
        show('success', '📷 Fotoğraf kaydedildi!')
        setTimeout(() => { window.location.href = window.location.pathname }, 700)
      } else {
        show('error', 'Kaydedilemedi')
      }
            ...
            <div className="rounded-2xl p-6 lg:sticky lg:top-20" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
              ...
              {currentQuest && (
                <div className="flex items-center justify-center gap-2 mb-3 px-3 py-1.5 rounded-full" style={{ background: `${currentQuest.color}15`, border: `1px solid ${currentQuest.color}40` }}>
                  ...
                  <span className="text-xs font-semibold" style={{ color: currentQuest.color }}>Lv {level} · {currentQuest.title}</span>
                </div>
              )}

              {user.bio && <p className="text-sm text-center leading-relaxed mb-4" style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-body)' }}>{user.bio}</p>}

              <div className="grid grid-cols-3 gap-2 mb-4 py-3 rounded-xl" style={{ background: 'var(--bg-subtle)' }}>
                ...
                {isOwnProfile ? (
                  <>
                    <button onClick={() => setEditOpen(true)}
                      className="w-full py-2.5 rounded-xl text-sm font-semibold text-white" style={{ background: '#1D9E75' }}>
                      ✏️ Profili Düzenle
                    </button>
                    <button onClick={() => setShowAvatarBuilder(p => !p)}
                      className="w-full py-2 rounded-xl text-sm font-medium border" style={{ borderColor: 'var(--border)', color: 'var(--text)' }}>
                      🎨 Karakter Tasarla
                    </button>
                    <Link href="/ayarlar" className="block w-full py-2 rounded-xl text-sm font-medium border text-center" style={{ borderColor: 'var(--border)', color: 'var(--text)' }}>
                      ⚙️ Ayarlar
                    </Link>
                  </>
                ) : (
                  <FollowButton targetId={user.id} isOwn={isOwnProfile} onChange={load} />
                )}
              </div>
            </div>
          </aside>
          ...
          <div className="space-y-6">
            {isOwnProfile && isNewUser && (
              <div className="rounded-2xl p-6" style={{ background: 'linear-gradient(135deg, rgba(29,158,117,0.15), rgba(15,110,86,0.1))', border: '1px solid rgba(29,158,117,0.3)' }}>
                ...
                <div className="flex gap-2 flex-wrap">
                  ...
                  <Link href="/katkida-bulun" className="px-4 py-2 rounded-xl text-sm font-semibold border" style={{ borderColor: 'var(--border)', color: 'var(--text)' }}>✍️ İçerik Ekle</Link>
                </div>
              </div>
            )}
            ...
            {showAvatarBuilder && isOwnProfile && (
              <AvatarBuilder
                ...
                onSaveBitmoji={saveAvatarConfig}
                onSavePhoto={savePhotoUrl}
            ...
            <ProfileTabs posts={posts} userId={params.id} />
          </div>
        </div>
      </div>
